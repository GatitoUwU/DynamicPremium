...
public class MySQL implements Database {
    ...
    public void loadDatabase(DatabaseManager databaseManager) {
        ...
        try {
            ...
            ProxyServer.getInstance().getConsole().sendMessage(c("&cDynamicPremium &8> &7Connected to MySQL"));
        } catch (Exception e) {
            e.printStackTrace();
            ProxyServer.getInstance().getConsole().sendMessage(c("&cDynamicPremium &8> &cSend this error to gatogamer#1111!"));
            ProxyServer.getInstance().getConsole().sendMessage(c("&cDynamicPremium &8> &cOh no, I can't connect to MySQL!"));
        }
    }
